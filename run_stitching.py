...
def stitching_row(start_index=(0, 0, 0), row_count=0, step=512, total_shape=(1024, 2304, 1792), maxlabel=maxlabel,
    ...
    current_shape = (
    min(total_shape[0] - start_index[0], step + 256), min(total_shape[1] - start_index[1], step + 256), total_shape[2])
...
def run_stitching(filename_raw=None, dataset_raw='/t00000/s00/0/cells', output_folder =None, input_folder=None,
    ...
    if not output_folder:
        output_folder = '/g/schwab/Viktoriia/src/source/upscaling_results/whole_dataset_mito/'  # + s4a2_mc_z_y_x_shape.h5
    if not input_folder:
